...
public class PersianRug {
	...
	private static void persianRug(
		...
		if (size < 0.006){
			...
			StdDraw.line(llx, lly+size, llx+size, lly+size);
			StdDraw.setPenColor(palette[east]);
			StdDraw.line(llx+size, lly+size, llx+size, lly);
			StdDraw.setPenColor(palette[south]);
			StdDraw.line(llx, lly, llx+size, lly);
			StdDraw.setPenColor(palette[west]);
			StdDraw.line(llx, lly, llx, lly+size);
			return;
		}
		
		int m = 2*(north+east+south+west+1)%12;
		// lower left
		persianRug(palette, llx,lly, size/2, m, m, south, west);
		// lower right
		persianRug(palette, llx + size/2, lly, size/2, m, east, south, m);
		// upper left
		persianRug(palette, llx, lly + size/2, size/2, north, m, m, west);
		// upper right
		persianRug(palette, llx + size/2, lly + size/2, size/2, north, east, m, m);
