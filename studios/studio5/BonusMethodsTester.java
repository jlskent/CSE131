 ...
 *
 */
public class BonusMethodsTester {
	...
	public void testSumDownBy2() {
		...
		if(BonusMethods.sumDownBy2(0) != 0)
		{
			printError("sumDownBy2 error when n = 0: Check to ensure that you cover all cases for n >= 0", 0, BonusMethods.sumDownBy2(0));
		}
		assertEquals(0, BonusMethods.sumDownBy2(0));
		...
		for(int i = 0; i < 100; i++)
		{
			...
			int sum = 0;
			int calcSum = BonusMethods.sumDownBy2(n);
			int nCopy = n;
	...
	public void testHarmonicSum() {
		...
		for(int i = 0; i < 100; i++)
		{
			...
			double calcSum = BonusMethods.harmonicSum(n);
			double sum = 0;
			int nCopy = n;
	...
	public void testGeometricSum() {
		...
		for(int i = 0; i < 100; i++)
		{
			...
			double calcSum = BonusMethods.geometricSum(k);
			double sum = 0;
	...
	public void testMultPos() {
		...
		for(int i = 0; i < 100; i++)
		{
			...
			checkMult(j, k, BonusMethods.multPos(j,k));
		}
	}
	...
	public void testMultAny() {
		...
		for(int i = 0; i < 100; i++)
		{
			...
			checkMult(j,k, BonusMethods.mult(j,k));
		}
	}
	...
	public void testExpt() {
		...
		for(int i = 0; i < 100; i++)
		{
			...
			int calcExpt = BonusMethods.expt(n, k);
			int expt = 1;
